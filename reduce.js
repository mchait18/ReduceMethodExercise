...
objects and a key and returns a new array with the value of 
...
function vowelCount(str) {
  return str
    .toLowerCase()
    .split("")
    .reduce(function (vowelObj, nextLetter) {
      if ("aeiou".indexOf(nextLetter) !== -1) {
        if (!vowelObj[nextLetter]) {
          vowelObj[nextLetter] = 1;
        } else {
          vowelObj[nextLetter] += 1;
        }
      }
...
function addKeyAndValue(arr, key, value) {
  return arr.reduce(function (acc, nextObj, indx) {
    acc[indx][key] = value;
    return acc;
  }, arr);
}
